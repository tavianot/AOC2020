...
let  part1list = [|
    ...
    "10-11 x: xxxxxxxxxcv"
    |]
let helper = part1list |> Array.toList
...
let getPolicyFromPortions (ranges:string) (letter:string) : Policy =
    let rangeList = ranges.Split[|'-'|]
    match rangeList with 
    | [|a;b|] -> {
                    letter = letter;
                    range = {min= (a|> int); max = (b |> int)}
                } 
    | _ -> failwith("not covering these cases yet")


let getPolicyFromString (policy:string) =
    match policy.Split[|' '|] with
    | [|a;b|] -> getPolicyFromPortions a b
    | _ -> failwith("Too many spaces")

let getPasswordTest (policy:Policy) (password:string) =
 {password=password; policy=policy} 

let isValidPassword (test:PasswordTest)=  // this comparison is used for part 1
    countSubstring test.password test.policy.letter
    |> isInRange test.policy.range.min test.policy.range.max

let getPolicyPassword (fullString:string) : PasswordTest =
    match fullString.Split[|':'|] with
    | [|a;b|] -> (getPasswordTest  (getPolicyFromString a) b)
    | _ -> failwith ("something is wrong")

let xor a b =
    (a ||b ) && not (a && b)
let isLetterAtPosition (test:PasswordTest) =
      xor  (test.password.[test.policy.range.min] = test.policy.letter.[0])  (test.password.[test.policy.range.max] = test.policy.letter.[0])

let CheckPasswords  lines  compare =
    List.map getPolicyPassword lines
    |> Seq.filter compare
    |> Seq.length


//part1
let part1 = CheckPasswords helper isValidPassword
//part2
let part2 = CheckPasswords helper isLetterAtPosition
